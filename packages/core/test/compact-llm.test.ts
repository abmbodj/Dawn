...
describe("compactViaLlm", () => {
  test("returns original messages unchanged when fewer than 2 groups", async () => {
    ...
    const { messages, summary } = await compactViaLlm(msgs, "anthropic/claude-haiku-4-5", fakeCatalog, fakeConfig)
  ...
  test("returns fewer messages after compaction (or fallback template)", async () => {
    ...
    const { messages: compacted, summary } = await compactViaLlm(msgs, "anthropic/claude-haiku-4-5", fakeCatalog, fakeConfig)
  ...
  test("keeps the newest half of turns verbatim", async () => {
    ...
    const { messages: compacted } = await compactViaLlm(msgs, "anthropic/claude-haiku-4-5", fakeCatalog, fakeConfig)
